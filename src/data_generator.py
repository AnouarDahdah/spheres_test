...
class SDFGenerator:
    ...
    def generate_sphere_sdf(self, center, radius):
        ...
        grid_points = self.grid.reshape(-1, 3)
        distances = torch.norm(grid_points - center, dim=1)
        distances = distances.reshape(self.grid_size, self.grid_size, self.grid_size)
